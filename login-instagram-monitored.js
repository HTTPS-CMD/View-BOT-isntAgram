...
// اضافه: ماژول مشاهده ویدئو
const { watchVideoWithDriver, watchVideoWithCookies } = require("./watch-video-after-login");
...
function loadAccounts() {
  ...
  try {
    ...
    if (!Array.isArray(parsed)) throw new Error("accounts.json باید آرایه باشد");
    const bad = parsed.filter(a => !a.username || !a.password);
    if (bad.length) {
      console.error("❌ برخی حساب‌ها username یا password ندارند:", bad);
      process.exit(1);
    }
    return parsed;
  } catch (e) {
    console.error("❌ خطا در خواندن یا پارس accounts.json:", e.message || e);
    process.exit(1);
  }
}
  ...
  try {
    ...
    try {
      ...
      if (!/login/i.test(url0)) {
        ...
        // try to watch video (driver is open and logged in)
        try {
          await watchVideoWithDriver(driver, username);
        } catch (e) {
          try { await watchVideoWithCookies(username); } catch {}
        }
        await driver.quit();
    ...
    if (!/login/i.test(url1)) {
      ...
      // ====== NEW: try watch video using the current driver ======
      try {
        await watchVideoWithDriver(driver, username);
      } catch (e) {
        // fallback: if using driver fails, try with cookies (new driver)
        try {
          await watchVideoWithCookies(username);
        } catch (ee) {
          console.warn("watch video failed for", username, ee?.message || ee);
        }
      }
      // ================================================
  ...
  for (let attempt = 1; attempt <= MAX_LOGIN_ATTEMPTS; attempt++) {
    ...
    if (res.challenge) {
      ...
      if (ok) {
        ...
        try {
          // watch video using the same open driver (human-in-loop session)
          await watchVideoWithDriver(driver, account.username);
        } catch (e) {
          // ignore and fallback
          try { await watchVideoWithCookies(account.username); } catch {}
        }
        try { await driver.quit(); } catch {}
